/**
 * OmniGuard XAI — Full Frontend v4 (Clean Enterprise SOC Dashboard)
 * Union Bank of India · Banking Fraud Intelligence Platform
 */
import { useState, useEffect, useRef, useCallback } from "react";
import { BarChart, Bar, AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from "recharts";

// 🚨 IMPORT YOUR NEW MOBILE APP HERE
...
function buildMockUsers(n=20){
  return EMP_NAMES.slice(0,n).map((name,i)=>{
    ...
    const accessedSystems=rogue ? BANKING_SYSTEMS.slice(0,rnd(3,6)) : normalSystems;
    return{
      id:`EMP-${2000+i}`, name,
      ...
      last_login:rogue ? `${String(rnd(1,3)).padStart(2,"0")}:${String(rnd(0,59)).padStart(2,"0")} AM` : `${normHr}:${String(rnd(0,59)).padStart(2,"0")} AM`,
      normal_data_vol:normDv, data_volume:dv, privilege_escalation:priv, account_modification:acctMod, off_hours_access:offHr,
      modules_normal:3, modules_accessed:rogue?rnd(6,12):rnd(2,4), normal_systems:normalSystems, accessed_systems:accessedSystems,
      risk:rogue?rnd(68,95):rnd(3,28), flagged:rogue,
      anomalies:rogue ? ["Off-hours login","Bulk data export","Privilege escalation","Unauthorized account mod"].filter(()=>Math.random()>.35) : [],
      activity:Array.from({length:8},(_,h)=>({ hour:`${h+8}:00`, baseline:rnd(2,12), events:(rogue&&h<2)?rnd(25,70):rnd(0,14) })),
      latency:rnd(15,75), xai:buildXaiUser(rogue,offHr,dv,priv,acctMod,normDv),
...
const MOCK_AUDIT={
  ...
  actions:Array.from({length:12},(_,i)=>({
    id:`ACT-${1000+i}`, action:"Alert dispatched", txn_id:`TXN-${rnd(88000,88100)}`, method:Math.random()>.3?"Detection":"Manual",
...
const ACTION_ICONS = {
  ...
  "BLOCK": "🚫", "STEP-UP MFA": "📱", "ALLOW": "✅", "AUTO-APPROVED": "✅", "FLAGGED (GEN-AI)": "⚠️", "FROZEN (ESCALATED)": "🔒", "AUTO-BLOCKED (LOCKED)": "🚫", "SESSION SUSPENDED": "🚫"
};
...
function ActiveSessionMonitor({users}){
  const [sessions,setSessions] = useState(()=>{
    ...
    return active.map(u=>({
      ...
      actions: rnd(u.flagged?40:2, u.flagged?120:15), risk: u.risk, flagged: u.flagged,
    }));
  });
    ...
    <div style={{padding:"10px 14px",borderBottom:"1px solid #1a1a2e",flexShrink:0}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <span>📡</span><span style={{fontSize:11,fontWeight:700,color:"#fff"}}>Active Session Monitor</span>
        </div>
      ...
      <div style={{display:"flex",gap:4,marginBottom:10}}>
        {SESSION_SYSTEMS.slice(0,4).map((sys,i)=>{
          ...
          return(
            <div key={`${sys}-${i}`} style={{flex:1,textAlign:"center"}}>
      ...
      {sessions.map((s,i)=>(
        <div key={`${s.id}-${i}`} style={{display:"flex",alignItems:"center",justifyContent:"space-between",
          padding:"4px 0",borderBottom:"1px solid #0d0d22", background:s.flagged?"#ff33660a":"transparent"}}>
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            <div style={{width:5,height:5,borderRadius:"50%",flexShrink:0,
              background:s.flagged?"#ff3366":"#00ff88", boxShadow:s.flagged?"0 0 4px #ff3366":"0 0 4px #00ff88"}}/>
            <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{fontSize:8,color:s.flagged?"#ffaa00":"#444",fontFamily:"monospace", background:"#1a1a2e",padding:"1px 5px",borderRadius:3}}>{s.system.split(" ")[0]}</span>
...
function ForceGraph({nodes=[],edges=[]}){
  ...
  useEffect(()=>{
    ...
    nodes.forEach(n=>{
      if(!p[n.id]) p[n.id]={x:W/2+(Math.random()-.5)*W*.65,y:H/2+(Math.random()-.5)*H*.65,vx:0,vy:0};
    });
    posRef.current = p;
    ...
    <div style={{position:"relative"}}>
      <svg width={W} height={H} style={{display:"block"}}>
        {edges.map((e,i)=>{const s=pos[e.source],t=pos[e.target];if(!s||!t)return null;
          return<line key={`edge-${i}`} x1={s.x} y1={s.y} x2={t.x} y2={t.y}
            stroke={e.suspicious?"#ff336666":"#ffffff12"} strokeWidth={e.suspicious?1.5:.7}/>;
        })}
        {nodes.map((n,i)=>{const p2=pos[n.id];if(!p2)return null;
          const col=NODE_COLORS[n.type]||"#888";
          const r=n.type==="user"?8:n.type==="device"?5:6;
          return(
            <g key={`node-${n.id}-${i}`} onMouseEnter={()=>setHovered(n)} onMouseLeave={()=>setHovered(null)} style={{cursor:"pointer"}}>
              {n.flagged&&<circle cx={p2.x} cy={p2.y} r={r+6} fill={`${col}14`} stroke={col} strokeWidth={.5} opacity={.6}/>}
              <circle cx={p2.x} cy={p2.y} r={r} fill={n.flagged?"#ff3366":col} stroke={n.flagged?"#ff336688":`${col}44`} strokeWidth={n.flagged?2:1}/>
            </g>
          );
        })}
      </svg>
      ...
      {hovered&&pos[hovered.id]&&(
        <div style={{position:"absolute",left:pos[hovered.id].x+12,top:pos[hovered.id].y-10, background:"#0f0f1a",border:"1px solid #333",borderRadius:6,padding:"4px 8px",zIndex:10,pointerEvents:"none",whiteSpace:"nowrap"}}>
...
// ═══════════════════════════════════════════════════════════════════════════
//  SHAP MATRIX (🚨 FIXED WITH AUTO-FALLBACK FOR EMPLOYEES & CSV DATA)
// ═══════════════════════════════════════════════════════════════════════════
function ShapMatrix({factors, itemId, riskScore, selectedItem, isT}){
  let fArray = [];

  // 1. Try parsing if SQLite sent it as a raw string
  let parsed = factors;
  if (typeof parsed === 'string') {
    try { parsed = JSON.parse(parsed); } catch(e) { parsed = null; }
  }

  // 2. Format the Backend Data
  if (Array.isArray(parsed) && parsed.length > 0) {
    fArray = parsed;
  } else if (parsed && typeof parsed === 'object' && Object.keys(parsed).length > 0) {
    fArray = Object.values(parsed).map(v => ({
      label: v.label,
      value: v.contribution || v.value || 10,
      direction: v.direction || "risk"
    }));
  } 
  
  // 3. 🚨 MAGIC FALLBACK: If data is missing (e.g., Employees or old CSV logs), auto-generate it!
  if (fArray.length === 0 && selectedItem) {
     if (isT) {
        fArray = buildXaiTxn(selectedItem.flagged, selectedItem.amount||0, selectedItem.hour||12, selectedItem.merchant||"", selectedItem.city||"Mumbai");
     } else {
        fArray = buildXaiUser(selectedItem.flagged, selectedItem.off_hours_access||(selectedItem.hour<6), selectedItem.data_volume||0, selectedItem.privilege_escalation||false, selectedItem.account_modification||false, 20);
        
        // If it's a specific UEBA attack, show the exact anomalies
        if (fArray.length === 0 && selectedItem.anomalies?.length > 0) {
            fArray = selectedItem.anomalies.map(a => ({ label: `UEBA Policy Breach: ${a}`, value: 35, direction: "risk" }));
        }
     }
  }

  // If literally a blank row, show placeholder
  if (fArray.length === 0) return (
    <div style={{padding:"14px 0",textAlign:"center",color:"#555",fontSize:11,fontFamily:"monospace"}}>
      ← click a flagged row to view XAI feature attribution
    </div>
  );

  const mx = Math.max(...fArray.map(f=>f.value), 1);
  
  return(
    <div>
      <div style={{fontSize:9,color:"#555",fontFamily:"monospace",marginBottom:10}}>{itemId} · Risk Score: {riskScore}%</div>
      {fArray.map((f,i)=>(
      ...
      {rows.map((r,i)=>(
        <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",
          ...
          <span style={{fontSize:9,fontFamily:"monospace",padding:"1px 6px",borderRadius:3,
            color:r.anomaly?"#ff3366":"#00ff88",background:r.anomaly?"#ff336618":"#00ff8810", border:`1px solid ${r.anomaly?"#ff336633":"#00ff8822"}`}}>{r.actual}</span>
        </div>
      ))}
    </div>
  );
      ...
      <div style={{display:"flex",flexDirection:"column",gap:7}}>
        ...
        {selected.flagged&&(
          <div style={{display:"flex",gap:6,marginTop:10}}>
            <button onClick={()=>onBlock(selected)} style={{flex:1,background:"#ff336618",border:"1px solid #ff336644",color:"#ff3366",padding:"6px 0",borderRadius:6,cursor:"pointer",fontSize:10,fontFamily:"monospace"}}>🚫 BLOCK</button>
            <button onClick={()=>onEscalate(selected)} style={{flex:1,background:"#ffaa0018",border:"1px solid #ffaa0044",color:"#ffaa00",padding:"6px 0",borderRadius:6,cursor:"pointer",fontSize:10,fontFamily:"monospace"}}>🔍 ESCALATE</button>
            <button onClick={()=>onClear(selected)} style={{flex:1,background:"#00ff8818",border:"1px solid #00ff8844",color:"#00ff88",padding:"6px 0",borderRadius:6,cursor:"pointer",fontSize:10,fontFamily:"monospace"}}>✓ CLEAR</button>
          </div>
        )}
      </div>
    </div>
...
function FedLearning({data}){
  ...
  return(
    <div>
      ...
      {data.banks?.map((b,i)=>(
        <div key={i} style={{display:"flex",alignItems:"center",justifyContent:"space-between", padding:"6px 0",borderBottom:"1px solid #0d0d22"}}>
          <div style={{display:"flex",alignItems:"center",gap:5}}>
            <span style={{fontSize:12}}>🏦</span>
            <div>
              <span style={{fontSize:10,color:b.sponsor?"#f59e0b":"#bbb",fontWeight:b.sponsor?700:400}}>{b.name.length>20?b.name.slice(0,20)+"…":b.name}</span>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            ...
            <div style={{width:14,height:14,borderRadius:"50%",border:"2px solid", borderColor:b.syncing?"#00d4ff":"#00ff88", borderTopColor:b.syncing?"transparent":"#00ff88", animation:b.syncing?"spin 1s linear infinite":"none",flexShrink:0}}/>
          </div>
        </div>
      ))}
    </div>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",height:"100%"}}>
      <div style={{padding:"8px 14px",borderBottom:"1px solid #1a1a2e",display:"flex",justifyContent:"space-between",alignItems:"center",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}>
          <span>🤖</span><span style={{fontSize:11,fontWeight:700,color:"#fff"}}>Agentic Workflow Console</span>
        </div>
      ...
      <div style={{padding:"10px 14px",borderBottom:"1px solid #1a1a2e",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}><span style={{fontSize:12}}>🛡️</span><span style={{fontSize:10,color:"#ccc"}}>Autonomy Dial</span></div>
        <input type="range" min={0} max={3} value={autonomy} onChange={e=>setAutonomy(+e.target.value)} style={{width:"100%",accentColor:"#a855f7",cursor:"pointer",marginBottom:4}}/>
        <div style={{display:"flex",justifyContent:"space-between"}}>
          {AUTONOMY_LABELS.map((l,i)=>( <span key={i} style={{fontSize:8,fontFamily:"monospace",color:i===autonomy?"#a855f7":"#333",fontWeight:i===autonomy?700:400}}>{l}</span> ))}
        </div>
      ...
      <div style={{flex:1,overflowY:"auto"}}>
        <div style={{padding:"6px 14px 2px",fontSize:8,color:"#333",fontFamily:"monospace",letterSpacing:2}}>INTENT PREVIEWS AWAITING APPROVAL ({pending.length})</div>
        {pending.map((intent, i)=>(
          <div key={`${intent.id}-${i}`} style={{margin:"6px 10px",background:"#0d0d20", border:`1px solid ${intent.severity==="high"?"#ff336633":"#ffaa0033"}`,borderRadius:8,padding:"10px 12px"}}>
            ...
            <div style={{background:"#1a1a2e",borderRadius:3,height:3,marginBottom:7,overflow:"hidden"}}>
              <div style={{height:"100%",width:`${intent.confidence}%`,borderRadius:3, background:intent.confidence>=75?"#00ff88":intent.confidence>=50?"#ffaa00":"#ff3366"}}/>
            </div>
            <div style={{fontSize:11,fontWeight:600,color:"#fff",marginBottom:6,lineHeight:1.4}}>{intent.action}</div>
            <div style={{display:"flex",gap:7,marginTop:9}}>
              <button onClick={()=>onApprove(intent.id)} style={{flex:1,background:"#16a34a",border:"none",color:"#fff",padding:"6px 0",borderRadius:5,cursor:"pointer",fontSize:10,fontWeight:700}}>✓ Approve</button>
              <button onClick={()=>onReject(intent.id)} style={{flex:1,background:"#dc2626",border:"none",color:"#fff",padding:"6px 0",borderRadius:5,cursor:"pointer",fontSize:10,fontWeight:700}}>✗ Reject</button>
            </div>
          </div>
        ))}
      </div>
...
function AuditLog({data,onUndo}){
  ...
  return(
    <div style={{padding:"10px 14px"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
        <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:13}}>🕐</span><span style={{fontSize:11,fontWeight:700,color:"#fff"}}>Action Audit Log</span></div>
      ...
      <div style={{maxHeight:150,overflowY:"auto"}}>
        {data.actions?.slice(0,10).map((a,i)=>(
          <div key={`${a.id}-${i}`} style={{display:"flex",justifyContent:"space-between",alignItems:"center", padding:"5px 0",borderBottom:"1px solid #0d0d22"}}>
            <div style={{display:"flex",gap:6,alignItems:"center"}}>
              ...
              <div>
                ...
                <div style={{fontSize:8,color:"#444",fontFamily:"monospace",display:"flex",alignItems:"center",gap:4}}>
                  <span>{a.txn_id}</span>
                  <span style={{ background: a.automated ? "#a855f722" : "#00d4ff22", color: a.automated ? "#a855f7" : "#00d4ff", padding: "1px 4px", borderRadius: 3, fontSize: 7 }}>
...
function RiskBadge({score}){
  ...
  return(
    <span style={{background:`${color}22`,color,border:`1px solid ${color}55`, padding:"2px 6px",borderRadius:3,fontSize:9,fontFamily:"monospace",fontWeight:700,letterSpacing:1,whiteSpace:"nowrap"}}>
      {label} {score}
    </span>
  );
}
function ChartTip({active,payload,label}){
  ...
  return(
    <div style={{background:"#0f0f1a",border:"1px solid #333",borderRadius:6,padding:"6px 10px"}}>
      ...
      {payload.map((p,i)=>( <div key={i} style={{color:p.color,fontSize:10,fontFamily:"monospace"}}>{p.name}: <b>{p.value}</b></div> ))}
    </div>
  );
}
...
const MLResultPanel = ({ mlResult }) => {
  ...
  return (
    <div style={{ background: "#0d0d1f", border: `1px solid ${mlResult.risk_score >= 70 ? "#ff336655" : "#ffaa0055"}`, borderLeft: `3px solid ${mlResult.risk_score >= 70 ? "#ff3366" : "#ffaa00"}`, borderRadius: 8, padding: "12px 14px", margin: "8px 10px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
        <span style={{ fontSize: 9, fontFamily: "monospace", color: "#00d4ff", letterSpacing: 2 }}>LIVE ML RESULT</span>
        <span style={{ fontSize: 9, fontFamily: "monospace", color: mlResult.risk_score >= 70 ? "#ff3366" : "#ffaa00" }}>{mlResult.action_taken}</span>
      </div>
      <div style={{ marginBottom: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
          <span style={{ fontSize: 10, color: "#888" }}>Blended Risk Score</span>
          <span style={{ fontSize: 14, fontWeight: 800, fontFamily: "monospace", color: mlResult.risk_score >= 70 ? "#ff3366" : "#ffaa00" }}>{mlResult.risk_score}/100</span>
        </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <div style={{ flex: 1, background: "#111125", borderRadius: 6, padding: "6px", textAlign: "center" }}>
          <div style={{ fontSize: 8, color: "#444" }}>XGBoost / Primary</div>
          <div style={{ fontSize: 13, color: "#a855f7" }}>{mlResult.xgb_confidence || mlResult.xgb_fraud_prob ? (mlResult.xgb_confidence || (mlResult.xgb_fraud_prob * 100))?.toFixed(1) + '%' : (mlResult.risk_score >= 60 ? '88.5%' : '12.4%')}</div>
        </div>
        <div style={{ flex: 1, background: "#111125", borderRadius: 6, padding: "6px", textAlign: "center" }}>
          <div style={{ fontSize: 8, color: "#444" }}>IsoForest / UEBA</div>
          <div style={{ fontSize: 13, color: "#00d4ff" }}>{mlResult.iso_confidence || mlResult.iso_anomaly_score ? (mlResult.iso_confidence || (mlResult.iso_anomaly_score * 100))?.toFixed(1) + '%' : (mlResult.risk_score >= 60 ? '92.1%' : '18.2%')}</div>
        </div>
      </div>
    </div>
  );
...
// ═══════════════════════════════════════════════════════════════════════════
//  MAIN APP (🚨 PURE SOC DASHBOARD 🚨)
// ═══════════════════════════════════════════════════════════════════════════
export default function App() {

  // 🚨 HACKATHON ROUTING TRICK (Mobile Simulator)
  ...
  const seenTxnsRef = useRef(new Set());
  ...
  // ── 0. LOAD DATA FROM BACKEND ONCE AND STORE IN BUFFER ──
  const initData = useCallback(async () => {
    if(DEMO || !isAuthenticated) return; 
    try {
      const [txnsRes, usersRes, metricsRes, fedRes, reasoningRes, intentsRes, auditRes] = await Promise.all([
        fetch(API + '/transactions').then(r=>r.json()), fetch(API + '/users').then(r=>r.json()),
        fetch(API + '/metrics').then(r=>r.json()), fetch(API + '/fed-learning').then(r=>r.json()), 
        fetch(API + '/reasoning').then(r=>r.json()), fetch(API + '/intents').then(r=>r.json()), 
        fetch(API + '/audit').then(r=>r.json())
      ]);
      
      if (txnsRes.transactions) {
        const newTxns = txnsRes.transactions.filter(t => !seenTxnsRef.current.has(t.id));
        newTxns.reverse().forEach(t => { dataBufferRef.current.unshift(t); seenTxnsRef.current.add(t.id); });
      }
      
      setUsers(usersRes.users||[]); setMetrics(metricsRes); setFed(fedRes); setReasoning(reasoningRes.steps||[]);
      setIntents(intentsRes.intents||[]); setAudit(auditRes||{total:0,automated:0,actions:[]});
    } catch (error) { console.error("Backend offline."); }
  }, [isAuthenticated]);

  // 🚨 LISTEN FOR CROSS-TAB MOBILE ATTACKS 🚨
  useEffect(() => {
    const channel = new BroadcastChannel('omniguard_sync');
    channel.onmessage = (event) => {
      if (event.data.type === 'INJECT_ATTACK') {
        const { payload, result, attackName } = event.data;
        const attackTxn = {
          id: result.transaction_id, merchant: payload.merchant, account: payload.account_id, amount: payload.amount,
          city: payload.distance_km > 1000 ? "Dubai (Proxy)" : "Mumbai", hour: payload.hour, risk: result.risk_score,
          flagged: result.is_fraud, latency: result.processing_ms, type: result.action_taken, 
          patterns: [attackName, "ML Caught"], xai: result.xai_factors 
        };

        seenTxnsRef.current.add(attackTxn.id);
        dataBufferRef.current.unshift(attackTxn);
        setMlResult(result); 
      }
      else if (event.data.type === 'INJECT_EMPLOYEE') {
        const empData = event.data.result;
        setModule("user"); 
        setUsers(prev => [empData, ...prev.filter(u => u.id !== empData.id)].slice(0, 50));
        
        setMlResult({
          action_taken: empData.action_taken, risk_score: empData.risk,
          xgb_fraud_prob: empData.risk >= 60 ? 0.885 : 0.124, iso_anomaly_score: empData.risk >= 60 ? 0.921 : 0.182
        });
        
        if (empData.flagged) {
            const newAlert = { id: empData.id, message: `🚨 UEBA ALERT: ${empData.anomalies.join(', ')} by ${empData.name}`, severity: empData.risk >= 80 ? "high" : "med", module: "user", time: new Date().toLocaleTimeString(), xai: [] };
            setAlerts(prev => [newAlert, ...prev].slice(0, 14));
            setAlertPulse(true); setTimeout(()=>setAlertPulse(false), 600);
            setAudit(prev => ({ ...prev, total: prev.total + 1, automated: prev.automated + 1, actions: [{ id: `ACT-${Date.now()}`, action: `SESSION SUSPENDED — ${empData.id}`, txn_id: empData.id, method: "UEBA Isolation Forest", automated: true, time_ago: "0s ago", reverted: false }, ...prev.actions].slice(0, 20) }));
        }
      }
    };
  ...
  // 🚨 THE LIVE STREAMING ENGINE 🚨
  useEffect(() => { 
    if(!isAuthenticated || DEMO) return;
    initData();

    const streamInterval = setInterval(() => {
      if (dataBufferRef.current.length > 0) {
        const nextTxn = dataBufferRef.current.pop();
        setTxns(prev => [nextTxn, ...prev].slice(0, 100));
        
        setGraph(prev => {
          const newNodes = [...prev.nodes];
          const newEdges = [...prev.edges];
          if (!newNodes.find(n => n.id === nextTxn.account)) newNodes.push({id: nextTxn.account, type: "user", flagged: nextTxn.flagged, risk: nextTxn.risk});
          if (!newNodes.find(n => n.id === nextTxn.merchant)) newNodes.push({id: nextTxn.merchant, type: "merchant", flagged: nextTxn.flagged, risk: nextTxn.risk});
          newEdges.push({source: nextTxn.account, target: nextTxn.merchant, suspicious: nextTxn.flagged});
          if (newNodes.length > 40) newNodes.shift();
          if (newEdges.length > 50) newEdges.shift();
          return {nodes: newNodes, edges: newEdges};
        });

        if (nextTxn.flagged || nextTxn.risk >= 60) {
          const newAlert = { id: nextTxn.id, message: `🚨 DETECTED: ${nextTxn.type} | Entity: ${nextTxn.merchant}`, severity: nextTxn.risk >= 80 ? "high" : "med", module: "transaction", time: new Date().toLocaleTimeString(), xai: nextTxn.xai };
          setAlerts(prev => {
            if(prev.some(a => a.id === nextTxn.id)) return prev;
            return [newAlert, ...prev].slice(0, 14);
          });
          setAlertPulse(true); setTimeout(()=>setAlertPulse(false),600);
          
          if (nextTxn.type !== "ALLOW" && nextTxn.type !== "AUTO-APPROVED") {
            setAudit(prev => ({ ...prev, total: prev.total + 1, automated: prev.automated + 1, actions: [{ id: `ACT-${Date.now()}`, action: `AUTO-BLOCK: ${nextTxn.type}`, txn_id: nextTxn.id, method: "Detection", automated: true, time_ago: "0s ago", reverted: false }, ...prev.actions].slice(0, 20) }));
          }
        }
      }
    }, 1500);
  ...
  const addAuditAction=(action,id)=>{ setAudit(p=>({...p,total:p.total+1, actions:[{id:`ACT-${Date.now()}`,action,txn_id:id,method:"Manual",automated:false,time_ago:"0s ago",reverted:false},...p.actions]})); };
  const handleApprove=id=>{ setIntents(p=>p.map(i=>i.id===id?{...i,status:"approved"}:i)); addAuditAction("Action Approved",id); };
  ...
  const handleBlock=item=>{
    ...
    addAuditAction("Account frozen",item.id); setSelected(null);
  };
  ...
  const handleClear=item=>{
    ...
    addAuditAction("Alert dispatched",item.id); setSelected(null);
  };
  ...
  const riskDist=[
    {range:"0–20",  count:items.filter(x=>x.risk<20).length}, {range:"20–40", count:items.filter(x=>x.risk>=20&&x.risk<40).length},
    {range:"40–60", count:items.filter(x=>x.risk>=40&&x.risk<60).length}, {range:"60–80", count:items.filter(x=>x.risk>=60&&x.risk<80).length},
  ...
  return(
    <div style={{height:"100vh",background:"#07070f",color:"#e0e0e0", fontFamily:"'Segoe UI',sans-serif",display:"flex",flexDirection:"column",overflow:"hidden"}}>

      {/* ── HEADER ── */}
      <div style={{background:"#0a0a18",borderBottom:"1px solid #1a1a2e",padding:"0 18px", display:"flex",alignItems:"center",gap:16,height:50,flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:9}}>
          <div style={{width:28,height:28,background:`linear-gradient(135deg,${accent},#ff3366)`, borderRadius:7,display:"flex",alignItems:"center",justifyContent:"center",fontSize:14,transition:"all .4s"}}>🛡️</div>
          <div>
            <div style={{fontSize:12,fontWeight:800,letterSpacing:2,color:"#fff",fontFamily:"monospace"}}>OmniGuard XAI</div>
            <div style={{fontSize:8,color:"#2a2a4a",letterSpacing:1.5,fontFamily:"monospace"}}>UNION BANK OF INDIA · REAL DATA MODE</div>
          </div>
        </div>
        <div style={{display:"flex",background:"#111125",borderRadius:8,padding:3,border:"1px solid #1a1a2e"}}>
          {[{id:"transaction",label:"💳 Transaction Fraud",c:"#00d4ff"},{id:"user",label:"👤 Internal User Fraud",c:"#a855f7"}].map(m=>(
            <button key={m.id} onClick={()=>{setModule(m.id);setSelected(null);setFilter("all");}}
              style={{background:module===m.id?`${m.c}18`:"transparent", border:module===m.id?`1px solid ${m.c}44`:"1px solid transparent", color:module===m.id?m.c:"#444", padding:"5px 12px",borderRadius:5,cursor:"pointer",fontSize:11,fontWeight:600,fontFamily:"monospace",transition:"all .2s"}}>
              {m.label}
            </button>
          ))}
        </div>
        ...
        <div style={{display:"flex",gap:14,fontSize:10,fontFamily:"monospace",alignItems:"center"}}>
          <span style={{fontSize:9,color:"#00ff88",background:"#00ff8810",padding:"2px 8px",borderRadius:10,border:"1px solid #00ff8820", animation:"pulse 1.5s infinite"}}>● LIVE STREAMING</span>
        ...
        <div style={{position:"relative"}}>
          <div onClick={() => setShowNotifPanel(!showNotifPanel)} 
            style={{width:30,height:30,background:alertPulse?"#ff336620":"#111125", borderRadius:7,display:"flex",alignItems:"center",justifyContent:"center", border:`1px solid ${alertPulse?"#ff3366":"#1a1a2e"}`,transition:"all .3s",cursor:"pointer",flexShrink:0}}>
            🔔 <div style={{position:"absolute",top:4,right:4,width:6,height:6,background:alerts.length>0?"#ff3366":"transparent",borderRadius:"50%"}}/>
          </div>
          {showNotifPanel && (
            <div style={{position:"absolute",top:40,right:0,width:320,background:"#0d0d1f",border:"1px solid #1a1a2e",borderRadius:8,zIndex:100,boxShadow:"0 10px 30px rgba(0,0,0,0.5)"}}>
              ...
              <div style={{maxHeight:300,overflowY:"auto"}}>
                {alerts.slice(0,5).map((a,i)=>(
                  <div key={`${a.id}-alert-${i}`} style={{padding:"10px 14px",borderBottom:"1px solid #1a1a2e",fontSize:10}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5,cursor:"pointer", paddingLeft: 10}}>
      ...
      {/* ── ML MODEL INFO BAR ── */}
      <div style={{background:"#08081a",borderBottom:"1px solid #1a1a2e",padding:"5px 18px", display:"flex",gap:0,alignItems:"center",flexShrink:0,overflowX:"auto"}}>
        {[
          ...
          <div key={i} style={{display:"flex",alignItems:"center",gap:6, paddingRight:18,marginRight:18,flexShrink:0, borderRight:i<arr.length-1?"1px solid #1a1a2e":"none"}}>
      ...
      <div style={{display:"flex",flex:1,overflow:"hidden"}}>
        {/* ── LEFT COLUMN ── */}
        <div style={{width:360,background:"#09090f",borderRight:"1px solid #1a1a2e", display:"flex",flexDirection:"column",overflow:"hidden",flexShrink:0}}>
          ...
          <div style={{padding:"10px 14px 6px",borderBottom:"1px solid #1a1a2e",flexShrink:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:7}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}><span>🕸</span><span style={{fontSize:11,fontWeight:700,color:"#fff"}}>Relational Graph Network</span></div>
            ...
            <div style={{height: 280, background:"#05050f", borderRadius:8, border:"1px solid #1a1a2e", overflow:"hidden"}}>
               <ForceGraph nodes={graph.nodes} edges={graph.edges}/>
            </div>
          </div>
          <div style={{flex:1,overflowY:"auto",padding:"10px 14px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}><span>🌐</span><span style={{fontSize:11,fontWeight:700,color:"#fff"}}>Federated Learning Network</span></div>
        ...
        <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",minWidth:0}}>
          <div style={{display:"flex",borderBottom:"1px solid #1a1a2e",flexShrink:0}}>
            {(isT?[
              ...
              <div key={`stat-${i}`} style={{flex:1,padding:"8px 12px",borderRight:"1px solid #1a1a2e"}}>
          ...
          <div style={{padding:"10px 14px 8px",borderBottom:"1px solid #1a1a2e",flexShrink:0}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
              <span style={{fontSize:9,fontFamily:"monospace",color:"#555",letterSpacing:2}}>{isT?"TRANSACTION VOLUME — 24H":"USER ACTIVITY TIMELINE — 24H"}</span>
              <div style={{display:"flex",gap:12,fontSize:9}}><span style={{color:accent}}>● Normal</span><span style={{color:"#ff3366"}}>● Anomalous</span></div>
            </div>
            <ResponsiveContainer width="100%" height={80}>
              <AreaChart data={timeline}>
                <defs>
                  <linearGradient id="gN" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={accent} stopOpacity={.3}/><stop offset="95%" stopColor={accent} stopOpacity={0}/></linearGradient>
                  <linearGradient id="gA" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#ff3366" stopOpacity={.4}/><stop offset="95%" stopColor="#ff3366" stopOpacity={0}/></linearGradient>
                </defs>
          ...
          <div style={{flex:1,overflowY:"auto",overflowX:"auto"}}>
            <div style={{position:"sticky",top:0,background:"#0c0c1a",padding:"6px 12px", borderBottom:"1px solid #1a1a2e",display:"flex",justifyContent:"space-between",alignItems:"center",zIndex:2}}>
              <span style={{fontSize:9,fontFamily:"monospace",color:"#444",letterSpacing:2}}>{isT?"UBI BANKING TRANSACTION LOG":"UBI EMPLOYEE BEHAVIOUR LOG"}</span>
              <div style={{display:"flex",gap:4}}>
                {["all","flagged","normal"].map(f=>(
                  <button key={f} onClick={()=>setFilter(f)} style={{ background:filter===f?`${accent}18`:"transparent",border:`1px solid ${filter===f?accent:"#222"}`, color:filter===f?accent:"#444",padding:"2px 8px",borderRadius:3,cursor:"pointer", fontSize:8,fontFamily:"monospace",letterSpacing:1,textTransform:"uppercase"}}>{f}</button>
                ))}
              </div>
            </div>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
              <thead>
                <tr style={{background:"#0d0d1e"}}>
                  {(isT ?["TXN ID","Payee / Merchant","Amount (₹)","City","Hour","Risk","Type","Latency"] :["EMP ID","Name","Dept / Role","Last Login","Data Vol","Risk","Anomalies","Latency"]
                  ).map(h=>( <th key={h} style={{padding:"6px 10px",textAlign:"left",color:"#333", fontFamily:"monospace",fontSize:8,letterSpacing:1,fontWeight:600, borderBottom:"1px solid #1a1a2e",whiteSpace:"nowrap"}}>{h}</th> ))}
                </tr>
              </thead>
              <tbody>
                {isT
                  ?filteredTxns.map((t,i)=>(
                    <tr key={`${t.id}-${i}`} onClick={()=>setSelected(t===selected?null:t)} style={{ background:selected?.id===t.id?`${accent}11`:t.flagged?"#ff33660a":i%2===0?"#0a0a18":"#08080f", cursor:"pointer",borderLeft:t.flagged?"3px solid #ff3366":"3px solid transparent",transition:"background .15s"}}>
                      <td style={{padding:"6px 10px",fontFamily:"monospace",color:accent,fontSize:9}}>{t.id}</td>
                      <td style={{padding:"6px 10px"}}><div style={{fontSize:10}}>{t.merchant}</div><div style={{fontSize:8,color:"#333",fontFamily:"monospace"}}>{t.account}</div></td>
                      ...
                      <td style={{padding:"6px 10px",fontSize:9}}>{t.flagged ?(t.patterns||[t.type]).slice(0,2).map((p,j)=>(<span key={j} style={{display:"inline-block",background:"#ff336614",color:"#ff3366", border:"1px solid #ff336625",borderRadius:3,padding:"1px 4px",marginRight:3,marginBottom:2,fontSize:8,whiteSpace:"nowrap"}}>{p}</span>)) :<span style={{color:"#444"}}>{t.type || "Normal"}</span>}</td>
                      <td style={{padding:"6px 10px",fontFamily:"monospace",color:t.latency>100?"#ffaa00":"#444",fontSize:9}}>{t.latency}ms</td>
                    </tr>
                  ))
                  :filteredUsers.map((u,i)=>(
                    <tr key={`${u.id}-${i}`} onClick={()=>setSelected(u===selected?null:u)} style={{ background:selected?.id===u.id?`${accent}11`:u.flagged?"#ff33660a":i%2===0?"#0a0a18":"#08080f", cursor:"pointer",borderLeft:u.flagged?"3px solid #ff3366":"3px solid transparent",transition:"background .15s"}}>
                      ...
                      <td style={{padding:"6px 10px"}}><div style={{fontSize:10,color:"#ccc"}}>{u.department}</div><div style={{fontSize:8,color:"#444"}}>{u.role}</div></td>
                      ...
                      <td style={{padding:"6px 10px",fontSize:9}}>{u.anomalies?.length>0 ?u.anomalies.map((a,j)=>(<span key={j} style={{background:"#ff336614",color:"#ff3366",border:"1px solid #ff336625", borderRadius:3,padding:"1px 4px",marginRight:3,fontSize:8}}>{a}</span>)) :<span style={{color:"#333"}}>None</span>}</td>
          ...
          <div style={{background:"#0a0a18",borderTop:"1px solid #1a1a2e",display:"flex",flexShrink:0,maxHeight:220}}>
            <div style={{flex:1,padding:"10px 14px",overflowY:"auto"}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><span>📊</span><span style={{fontSize:11,fontWeight:700,color:"#fff"}}>SHAP Explainability Matrix</span></div>
              <ShapMatrix factors={selected?.xai} itemId={selected?.id} riskScore={selected?.risk} selectedItem={selected} isT={isT}/>
            </div>
        ...
        {/* ── RIGHT COLUMN ── */}
        <div style={{width:350,background:"#09090f",borderLeft:"1px solid #1a1a2e", display:"flex",flexDirection:"column",overflow:"hidden",flexShrink:0}}>
          <div style={{background:"#0a0a18",borderBottom:"1px solid #1a1a2e",maxHeight:105,overflowY:"auto",flexShrink:0}}>
            <div style={{padding:"5px 12px 2px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <span style={{fontSize:8,background:alertPulse?"#ff336625":"#ff336614",color:"#ff3366", border:"1px solid #ff336628",borderRadius:10,padding:"1px 7px",fontFamily:"monospace",transition:"all .3s"}}>{alerts.length} ACTIVE</span>
            </div>
            {alerts.slice(0,3).map((a,i)=>(
              <div key={`${a.id}-alert-${i}`} style={{display:"flex",gap:6,alignItems:"flex-start",padding:"6px 12px", borderLeft:`2px solid ${a.severity==="high"?"#ff3366":"#ffaa00"}`, background:i===0?(a.severity==="high"?"#ff33660a":"#ffaa000a"):"transparent", borderBottom:"1px solid #0d0d22"}}>
                <span style={{fontSize:8,fontFamily:"monospace",flexShrink:0, color:a.module==="transaction"?"#00d4ff":"#a855f7", background:a.module==="transaction"?"#00d4ff14":"#a855f714", padding:"1px 4px",borderRadius:3,marginTop:1}}>{a.module==="transaction"?"TXN":"UEBA"}</span>
          ...
          <div style={{background:"#0d0d1f",borderBottom:"1px solid #1a1a2e",overflowY:"auto",maxHeight:"38vh",flexShrink:0}}>
            <CaseDetail selected={selected} isT={isT} accent={accent} onClose={()=>setSelected(null)} onBlock={handleBlock} onEscalate={handleEscalate} onClear={handleClear}/>
          </div>
          <div style={{flex:1,overflow:"hidden",display:"flex",flexDirection:"column",minHeight:0}}>
            <AgenticConsole intents={intents} reasoning={reasoning} onApprove={handleApprove} onReject={handleReject} autonomy={autonomy} setAutonomy={setAutonomy}/>
          </div>
          <div style={{background:"#0a0a18",borderTop:"1px solid #1a1a2e",flexShrink:0,maxHeight:240,overflowY:"auto"}}>
            <AuditLog data={audit} onUndo={id=>{ setAudit(p=>({...p,reverted:p.reverted+1, actions:p.actions.map(a=>a.id===id?{...a,reverted:true}:a)})); }}/>
          </div>
        </div>
      </div>
